...
class MySampleConsensusModel : public pcl::SampleConsensusModel<pcl::PointXYZ> {
public:
    using pcl::SampleConsensusModel<pcl::PointXYZ>::SampleConsensusModel;

    virtual int getSampleSize() const = 0;
};

// 自定义的二次模型类
class SampleConsensusModelQuadratic : public MySampleConsensusModel {
public:
    using MySampleConsensusModel::MySampleConsensusModel;

    bool computeModelCoefficients(const std::vector<int> &samples, Eigen::VectorXf &model_coefficients) const override {
        if (samples.size() != 3) {
            PCL_ERROR("[pcl::SampleConsensusModelQuadratic::computeModelCoefficients] Invalid set of samples given (3 samples are required). Returning false.\n");
            return false;
        }
        ...
        for (size_t i = 0; i < 3; ++i) {
            const auto &point = (*input_)[samples[i]];
            A(i, 0) = point.x * point.x;  // x^2
            A(i, 1) = point.x;            // x
            A(i, 2) = 1.0;                // 常数项
            B(i) = point.y;
        }
        ...
        model_coefficients[0] = X(0);  // 二次项系数 a
        model_coefficients[1] = X(1);  // 一次项系数 b
        model_coefficients[2] = X(2);  // 常数项 c
    ...
    void getDistancesToModel(const Eigen::VectorXf &model_coefficients, std::vector<double> &distances) const override {
        ...
        for (size_t i = 0; i < indices_->size(); ++i) {
            const auto &point = (*input_)[(*indices_)[i]];
            double y_pred = model_coefficients[0] * point.x * point.x + model_coefficients[1] * point.x + model_coefficients[2];
            distances[i] = std::abs(point.y - y_pred);
        }
    }

    void selectWithinDistance(const Eigen::VectorXf &model_coefficients, double threshold, std::vector<int> &inliers) override {
        inliers.clear();
        for (size_t i = 0; i < indices_->size(); ++i) {
            const auto &point = (*input_)[(*indices_)[i]];
            double y_pred = model_coefficients[0] * point.x * point.x + model_coefficients[1] * point.x + model_coefficients[2];
            if (std::abs(point.y - y_pred) < threshold) {
...
private:
    void fitLine(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, pcl::ModelCoefficients::Ptr coefficients, pcl::PointIndices::Ptr inliers) {
        SampleConsensusModelQuadratic::Ptr model(new SampleConsensusModelQuadratic(cloud));
        pcl::SACSegmentation<pcl::PointXYZ> seg;
        seg.setOptimizeCoefficients(true);
        seg.setModelType(pcl::SACMODEL_CUSTOM);  // 使用自定义模型
        seg.setMethodType(pcl::SAC_RANSAC);
        seg.setDistanceThreshold(0.5);
        seg.setSampleConsensusModel(model);  // 设置自定义模型
        seg.setInputCloud(cloud);
        seg.segment(*inliers, *coefficients);

        // 检查是否找到内点
        if (inliers->indices.empty()) {
            std::cerr << "Could not estimate a quadratic curve model for the given dataset." << std::endl;
            return;
        }
        ...
        // 使用最小二乘法进行二次项曲线拟合
        if (cloud_inliers->points.size() < 3) {
            std::cerr << "Not enough points to fit a quadratic curve." << std::endl;
            return;
        }

        Eigen::MatrixXd A(cloud_inliers->points.size(), 3);
        Eigen::VectorXd B(cloud_inliers->points.size());

        for (size_t i = 0; i < cloud_inliers->points.size(); ++i) {
            A(i, 0) = cloud_inliers->points[i].x * cloud_inliers->points[i].x;  // x^2
            A(i, 1) = cloud_inliers->points[i].x;                             // x
            A(i, 2) = 1.0;                                                    // 常数项
        ...
        coefficients->values[0] = X(0);  // 二次项系数 a
        coefficients->values[1] = X(1);  // 一次项系数 b
        coefficients->values[2] = X(2);  // 常数项 c
    }
